...
## ⚠️ 重大變更中：正負號慣例翻正 (2026-08-19 發現，進行中)
...
**根本原因**：FEM 符號選反了。我們用 `FEM_近端=+wL²/12, FEM_遠端=-wL²/12`；
傳統教科書慣例（也是這次要改成的目標）是 `FEM_近端=-wL²/12, FEM_遠端=+wL²/12`。
只要把 FEM 符號換過來，算出來的每一個數值(θ、M)都跟傳統教科書例題逐項
精確一致（不只是大小，連正負號都對上）。
...
**決定**：全面改成傳統教科書慣例（FEM近端負、遠端正，順時針為正這句話
變成名符其實）。影響範圍：
- 三個 Case 的 FEM 公式全部要反過來
- `draw_bmd`/`draw_sfd`/`draw_tension_side`/`draw_deformed_shape` 裡
  「遠端要negate」的規則要整組換成「近端要negate」(因為M值全部反號，
  要維持同一個物理上正確的拉力側/變形圖，negate的對象要跟著換)
- Case-01 教學詳解(`teaching_breakdown`)裡寫死的具體數值要重算
- `describe_dof()` 的文字說明本身不用改("順時針為正"這句話終於變成真的)
- 所有 notebook (內嵌版+wget版，共8份以上) 都要重新產生、重新驗證

**執行順序**：wget 版先做完（方便在自己的 Linux 環境重複測試），
Case-03 優先（目前討論最深入），Case-01/02 接續補上。
`sd_framework.py` 的 `solve_and_report()`/`print_teaching_handout()`
同時精簡：手算詳解拿掉SFD/BMD(留給教學講義那個cell，避免重複)；
教學講義拿掉教學詳解評分要點文字，只留五張圖(結構受力→剪力→彎矩→
拉力側→變形圖)。
